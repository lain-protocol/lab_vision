...
conf.calDir = 'data/imagenet_train_tiny' ;
conf.dataDir = '/root/edrive/Work/Doctorado/2015-1/MachineVision/phog_lab7/example/results_02/' ;
conf.autoDownloadData = false;
conf.numTrain = 90 ; %for train tiny set
conf.numTest = 10 ; %for train tiny set (validation)
conf.numClasses = 50 ;
conf.numWords = 600 ;
conf.numSpatialX = [4 8] ;
conf.numSpatialY = [4 8] ;
...
conf.prefix = 'm14_imagenet_tiny_train' ;
conf.randSeed = 1 ;
...
%fullfile(conf.calDir, classes{ci}, '*.JPEG')
...
figure(2) ; clf;
subplot(1,2,1) ;
imagesc(scores(:,selTrain)) ; title('Scores-train') ;
set(gca, 'ytick', 1:length(classes), 'yticklabel', classes) ;
subplot(1,2,2) ;
imagesc(scores(:,selTest)*2) ; title('Scores-test') ;
set(gca, 'ytick', 1:length(classes), 'yticklabel', classes) ;
